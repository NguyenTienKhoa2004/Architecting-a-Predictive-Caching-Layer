...
>
> **Shared Architectural Principle — Decision/Serving Layer Separation:** Both this thesis and the article enforce a strict separation between the **Decision Layer** (AI inference) and the **Serving Layer** (user-facing delivery). The article notes that inline model inference costs 20–100ms per request — unacceptable latency at scale. Their solution: run ranking inference *ahead of time* and write results to a fast lookup store. This thesis applies the same principle: the LSTM runs inference *offline* to predict which keys will be hot in the next 15 minutes, then **pre-loads them into Redis** before users request them. At serving time, Redis responds directly from cache with **zero AI overhead** — no model inference occurs in the critical request path.
>
> **Key Difference — Scope & Data Source:** The article operates as a **live production** system at the web frontend layer with continuous streaming data refreshed weekly. This thesis is a **Proof-of-Concept offline** backend caching layer (standalone Redis), using a fixed 14-day dataset augmented from 7 days of Twitter traces via time-shifting and noise injection. The prediction target also differs: they predict *user content preferences*, while this thesis predicts *cache key access frequency*.
